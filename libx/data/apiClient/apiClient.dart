...
class ApiClient extends GetConnect {
  ...
  Future<bool> onboarding_signup(
    ...
    try {
      ...
      var response = await http.post(
        headers: header,
        Uri.parse(
          'https://ctl.elianaeliohotels.com/api/ctl/register?cache-buster=${fn_generateCacheBuster()}',
        ),
        body: jsonEncode({
          'firstname': firstName,
          'lastname': lastName,
  ...
  Future<http.Response> fetchNearbyVendors() async {
    // var token = await dataBase.getToken();
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
    final url = Uri.parse('$baseUrl/vendors/nearby');
    _logRequest('GET', url);
    final response = await http.get(
      url,
      headers: <String, String>{
  ...
  Future<http.Response> fetchCategory() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
    final url = Uri.parse('$baseUrl/categories/industries');
    _logRequest('GET', url);
    final response = await http.get(
      url,
      headers: <String, String>{
  ...
  Future<http.Response> fetchVendorByCategory(String type) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
    final url = Uri.parse('$baseUrl/vendors/nearby/$type');
    _logRequest('GET', url);
    final response = await http.get(
      url,
      headers: <String, String>{
  ...
  Future<http.Response> validateUserSignupEmail(
    ...
    final url = Uri.parse(
      'https://ctl.elianaeliohotels.com/api/ctl/validate-email',
    );
    _logRequest('POST', url, body: otpData);
    final response = await http.post(
      url,
