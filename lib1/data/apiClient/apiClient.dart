...
import 'package:ctluser/utils/snackbar_utils.dart';
...
class ApiClient extends GetConnect {
  ...
  var basePath = 'https://nksereke.elianaeliohotels.com/api';
  var baseUrl = 'https://nksereke.elianaeliohotels.com/api';
  ...
  Future<bool> onboarding_login(String email, String password) async {
    ...
    try {
      print("onboarding_login > $basePath/auth/login");

      var response = await post(
        '$basePath/auth/login?cache-buster=${fn_generateCacheBuster()}',
      ...
      if (response.statusCode == 200) {
        ...
        if (body.containsKey('token')) {
          await dataBase.saveToken(body['token']);
        }
  ...
  Future<Map<String, dynamic>?> register(
    ...
    try {
      print("register > $basePath/auth/register");

      var response = await post(
        '$basePath/auth/register?cache-buster=${fn_generateCacheBuster()}',
        {
          'name': name,
          'email': email,
          'phone': phone,
      ...
      myLog.log("Registration failed! ${error.toString()}");

      return null;
    }
  }

  Future<bool> verifyEmail(int userId, String otp) async {
    try {
      print("verifyEmail > $basePath/auth/verify-email");

      var response = await post(
        '$basePath/auth/verify-email?cache-buster=${fn_generateCacheBuster()}',
      ...
      myLog.log("Verification failed! ${error.toString()}");
  ...
  Future<bool> forgotPassword(String email) async {
    try {
      print("forgotPassword > $basePath/auth/forgot-password");

      var response = await post(
        '$basePath/auth/forgot-password?cache-buster=${fn_generateCacheBuster()}',
        {'email': email},
      );
      ...
      if (response.statusCode == 200) {
        SnackbarUtils.showSnackbarMessage(
          message: body['message'] ?? 'Reset code sent',
        );
        return true;
      } else {
        SnackbarUtils.showSnackbarError(
          "Forgot password failed",
          body['message'] ?? 'Unable to send reset code',
        );
        return false;
      }
    } catch (error) {
      SnackbarUtils.showSnackbarError(
        "Oops",
        "Forgot password failed! ${error.toString()}",
      );

      myLog.log("Forgot password failed! ${error.toString()}");
  ...
  Future<bool> resetPassword(
    ...
    try {
      print("resetPassword > $basePath/auth/reset-password");

      var response = await post(
        '$basePath/auth/reset-password?cache-buster=${fn_generateCacheBuster()}',
        {
          'email': email,
          'code': code,
          'password': password,
      ...
      SnackbarUtils.showSnackbarError(
        "Oops",
        "Reset password failed! ${error.toString()}",
      );

      myLog.log("Reset password failed! ${error.toString()}");
  ...
  Future<bool> onboarding_signup(
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        //Get.snackbar('Successs', 'An OTP has been sent to your email');
        ...
        // }
        myLog.log(jsonDecode(response.statusCode.toString()).toString());
      ...
      Get.snackbar("Oops", "Sign up failed! ${error.toString()}");
  ...
  Future<http.Response> resendOtp(Map<String, dynamic> customerData) async {
    final url = Uri.parse('$basePath/auth/resend-otp');
  ...
  Future<http.Response> forgotPassword(
    ...
    final url = Uri.parse('$basePath/auth/forgot-password');
    _logRequest('POST', url, body: customerData);
    final response = await http.post(
      url,
      headers: <String, String>{
  ...
  Future<http.Response> storeAddress(Map<String, dynamic> addressData) async {
    ...
    final url = Uri.parse('$basePath/customer/addresses');
  ...
  Future<http.Response> updateAddress(
    ...
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
    final url = Uri.parse('$basePath/customer/addresses/$id');
  ...
  Future<http.Response> resetPassword(Map<String, dynamic> customerData) async {
    final url = Uri.parse('$basePath/auth/reset-password');
    _logRequest('POST', url, body: customerData);
    final response = await http.post(
      url,
      headers: <String, String>{
  ...
  Future<http.Response> fetchNearbyVendors({
  ...
  Future<http.Response> fetchCategory() async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
    final uri = Uri.parse('$basePath/browse/categories');
    _logRequest('GET', uri);
    final response = await http.get(
      uri,
      headers: <String, String>{
  ...
  /// fetch vendor by category

  Future<http.Response> fetchVendorByCategory(String type) async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
    final uri = Uri.parse('$basePath/browse/vendors').replace(
      queryParameters: {
        'category_id': type,
        'latitude': '6.5244',
        'longitude': '3.3792',
  ...
  Future<http.Response> fetchCategoriesAndProducts(
    String company_id,
    String category_id,
  ) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
    final url = Uri.parse('$baseUrl/categories/vendors/3/59');
    _logRequest('GET', url);
    final response = await http.get(
      url,
      headers: <String, String>{
  ...
  Future<http.Response> fetchProductByCategory(
    String company_id,
    String category_id,
  ) async {
    var userId = await dataBase.getUserId();
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
    final url = Uri.parse('$baseUrl/products/3/59/4');
    _logRequest('GET', url);
    final response = await http.get(
      url,
      headers: <String, String>{
  ...
  Future<http.Response> fetchPromotion() async {
    var userId = await dataBase.getUserId();
    var companyId = await dataBase.getCompanyId();
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
    final url = Uri.parse('$baseUrl/promotions/3/59');
    _logRequest('GET', url);
    final response = await http.get(
      url,
      headers: <String, String>{
  ...
  /// verify pin
  Future<http.Response> verifyPin(Map<String, dynamic> addressData) async {
    //var token = await dataBase.getToken();
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
  ...
  /// reset pin
  Future<http.Response> resetPin(Map<String, dynamic> addressData) async {
    //var token = await dataBase.getToken();
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
    final url = Uri.parse('$baseUrl/pin/reset');
    _logRequest('POST', url, body: addressData);
    final response = await http.post(
      url,
      body: jsonEncode(addressData),
  ...
  /// request reset
  Future<http.Response> requestResetPin() async {
    //var token = await dataBase.getToken();
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
    final url = Uri.parse('$baseUrl/pin/request-reset');
    _logRequest('POST', url);
    final response = await http.post(
      url,
      //body: jsonEncode(addressData),
  ...
  /// add pin
  Future<http.Response> addPin(Map<String, dynamic> addressData) async {
    //var token = await dataBase.getToken();
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
  ...
  Future<http.Response> validateUserSignupEmail(
    ...
    final url = Uri.parse('$basePath/auth/verify-email');
    _logRequest('POST', url, body: otpData);
    final response = await http.post(
      url,
      headers: <String, String>{
  ...
  Future<http.Response> login(Map<String, dynamic> loginData) async {
    final url = Uri.parse('$basePath/auth/login');
  ...
  Future<http.Response> fetchBusinessCategory() async {
    if (Get.isSnackbarOpen) {
      // return Future.error('Snackbar is open');
      Get.closeAllSnackbars();
    }

    final url = Uri.parse('$baseUrl/types/industries');
    _logRequest('GET', url);
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? '';
  ...
  Future<http.Response> fetchAllOrders() async {
    final url = Uri.parse('$basePath/customer/orders');
    var token = await dataBase.getToken();
    _logRequest('GET', url);
    return _retryRequest(() async {
      final response = await http
  ...
  //create pin
  Future<http.Response> createPin(Map<String, dynamic> packData) async {
    final url = Uri.parse('$baseUrl/vendor/packs');
    var token = await dataBase.getToken();
    _logRequest('POST', url);
    return _retryRequest(() async {
